...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
         ...
         */
        List<SimpleGrantedAuthority> authorities = roles.stream().map(role -> "ROLE_" + role.getName())
                .map(SimpleGrantedAuthority::new).collect(Collectors.toList());

        // add privileges to authorities
        authorities.addAll(roles.stream().map(Role::getPrivileges).flatMap(Collection::stream)
                .map(Privilege::getName).map(SimpleGrantedAuthority::new).toList());
